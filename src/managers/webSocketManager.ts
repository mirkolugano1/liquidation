...
class WebSocketManager {
    ...
    async processLog(log: any, network: string) {
        ...
        switch (eventHash) {
            case "0x0559884fd3a460db3073b7fc896cc77986f16e378210ded43186175bf646fc5f": //AnswerUpdated
                ...
                if (!_.includes(aggregators, priceOracleAggregatorAddress)) {
            ...
            case "0xe6cd5d3f2cfd1bf39c09a4afc6cfbfbce2a3c2c5c9d64e7464b3a8b5c2eecb1e": //FlashLoan
                ...
                if (flashloanInitiator) addressesToAdd.push(flashloanInitiator);
                break;
            ...
            case "0xde6857219544bb5b7746f48ed30be6386fefc61b2f864cacf559893bf50fd951":
                ...
                if (log.topics.length > 3) addressesToAdd.push(log.topics[3]); //onBehalfOf The beneficiary of the deposit, receiving the aTokens
                break;
            ...
            case "0xa534c8dbe71f871f9f3530e97a74601fea17b426cae02e1c5aee42c96c784051": //Repay
                if (log.topics.length > 2) addressesToAdd.push(log.topics[2]); //user The beneficiary of the repayment, getting his debt reduced
                if (log.topics.length > 3) addressesToAdd.push(log.topics[3]); //repayer The address of the user initiating the repay(), providing the funds
                break;

            case "0x2b627736bca15cd5381dcf80b0bf11fd197d01a037c52b927a881a10fb73ba61": //Supply
                if (log.topics.length > 2) addressesToAdd.push(log.topics[2]); //user The address of the user
            ...
            case "0x3115d1449a7b732c986cba18244e897a450f61e1bb8d589cd2e69e6c8924f9f7": //Withdraw
                if (log.topics.length > 2) addressesToAdd.push(log.topics[2]); //user The address initiating the withdrawal, owner of aTokens
                if (log.topics.length > 3) addressesToAdd.push(log.topics[3]); //to Address that will receive the underlying
                break;
        ...
        if (addressesToAdd.length > 0) {
            ...
            //normalize addresses
            let normalizedAddressesToAdd = _.map(addressesToAdd, (address) =>
                common.normalizeAddress(address)
            );
    ...
    //#endregion processAaveEvent (Alchemy WebSockets)
